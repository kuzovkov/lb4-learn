...
export class TodoController {
  ...
  async getTodoList(
      @param.path.number('id') id: typeof Todo.prototype.id): Promise<TodoList> {
  ...
  @get('/hello/{name}', {
    ...
    parameters: [
      {'name': 'name', 'type': 'string', 'in': 'path', 'required': true, 'description': 'the name for greeting', 'example': 'sania'},
    ],
    responses: {
      '200': {
        description: 'Hello',
  ...
  async hello(
      @param.path.string('name') name: string): Promise<string> {
    return `Hello ${name}`;
  }
